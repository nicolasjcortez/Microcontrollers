...
def msg_bot(msg):
    keyboard = {"one_time_keyboard": True, "keyboard":[[{"text":"Abrir"},{"text":"Alarme"},{"text":"Ignorar"}]]}
    endereco = endereco_base + "/sendMessage"
    dados = {"chat_id": id_da_conversa, "text": msg, "reply_markup": keyboard}
...
def remover_acento(data):
    return ''.join(x for x in unicodedata.normalize('NFD', data) if unicodedata.category(x)!='Mn')
